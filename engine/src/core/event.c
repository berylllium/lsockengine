...
{
	uint16_t event_code;
	lise_darray listeners; // lise_on_event_cb
} lise_event_entry;

static lise_darray registered_events; // lise_event_entry
...
void lise_event_shutdown()
{
	...
	{
		lise_event_entry* entry = lise_darray_get(&registered_events, entry_index);

		lise_darray_free(&entry->listeners);
	}
...
void lise_event_register(uint16_t event_code)
{
	...
	entry.listeners = lise_darray_create(lise_on_event_cb);

	lise_darray_push_back(&registered_events, &entry);
}
...
void lise_event_fire(uint16_t event_code, lise_event_context ctx)
{
	for (int entry_index = 0; entry_index < registered_events.size; entry_index++)
	{
		...
		if (entry->event_code == event_code)
		{
			...
			{
				lise_on_event_cb* event_cb = lise_darray_get(&entry->listeners, listener);
...
void lise_event_add_listener(uint16_t event_code, lise_on_event_cb listener)
{
	for (int entry_index = 0; entry_index < registered_events.size; entry_index++)
	{
		...
		if (entry->event_code == event_code)
		{
			lise_darray_push_back(&entry->listeners, &listener);
